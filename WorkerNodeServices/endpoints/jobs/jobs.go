...
func MapJob(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error reading file, please check."))
		utils.SimpleFailStatus("Failed reading file in worker for mapper proc", w)
		return
	}
	...
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error creating stdin pipe for mapper proc"))
		utils.SimpleFailStatus("Failed creating stdin pipe for mapper proc", w)
		return
	}
...
func ShuffleShare(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error opening file for shuffle share"))
		utils.SimpleFailStatus("Error opening file for shuffle share", w)
		return
	}
	defer fd.Close()
	if err != nil {
...
func customShuffleFunction(w http.ResponseWriter, r *http.Request) {
	...
	fileBytes, err := io.ReadAll(file)
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error reading custom shuffle file, please check."))
		utils.SimpleFailStatus("Failed reading file in worker for shuffle proc", w)
		return
	}
	new_fd, err := os.Create("./" + handler.Filename)
	os.Chmod("./"+handler.Filename, 0777)
...
func ReduceJob(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error reading file, please check."))
		utils.SimpleFailStatus("Failed reading file in worker for reduce proc", w)
		return
	}
	...
	stdinPipe, err := cmd.StdinPipe()
	if err != nil {
		log.Println(color.Colorize(color.Red, "Error creating stdin pipe for reduce job"))
		utils.SimpleFailStatus("Failed creating stdin pipe for reduce job", w)
		return
	}
